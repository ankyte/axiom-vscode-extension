...
export class DriftDetectionEngine {
  public detect(repo: string, memories: MemoryRecord[], ruleSet?: ArchitectureRuleSet): DriftMemory[] {
  ...
  private observed(rule: ArchitectureRule, text: string): string | null {
    const matches = rule.observedPatterns.filter((pattern) => text.includes(pattern.toLowerCase()));
    return matches.length ? matches.join(', ') : null;
  }

  private makeFromRule(repo: string, rule: ArchitectureRule, observed: string): DriftMemory {
    return {
      id: hashId(`drift:${repo}:${rule.id}:${observed}`),
      type: 'DRIFT',
      ruleId: rule.id,
  ...
  private unique(drifts: DriftMemory[]): DriftMemory[] {
    const byKey = new Map<string, DriftMemory>();
