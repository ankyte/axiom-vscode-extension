...
class CopilotAdapter {
    ...
    async extractChunks(limit = 200, workspaceRoot) {
        ...
        for (const file of sessionFiles) {
            ...
            for (const textBlock of this.readMessageBlocks(raw)) {
                if (!this.isHighSignal(textBlock))
                    continue;
                if (!this.isRelevantToWorkspace(textBlock, workspaceRoot))
                    continue;
                const referencedFiles = this.extractFiles(textBlock);
                const relatedServices = this.extractServices(textBlock);
                chunks.push({
                    id: (0, utils_1.hashId)(`${file}:${textBlock}`),
                    source: 'copilot',
                    sessionId,
                    text: (0, utils_1.compactText)(textBlock, 420),
                    referencedFiles,
                    relatedServices,
                });
                if (chunks.length >= limit)
    ...
    async collectSessionFiles(chatDir) {
    ...
    isHighSignal(text) {
        ...
        if (lower.includes('vscode.prompt.instructions'))
            return false;
        if (lower.includes('agents.md') && !lower.includes('architecture') && !lower.includes('service'))
            return false;
        if (/^[-A-Za-z0-9+/=_]{140,}$/.test(text.trim()))
            return false;
    ...
    isRelevantToWorkspace(text, workspaceRoot) {
        if (!workspaceRoot)
            return true;
        const lower = text.toLowerCase();
